...
void setup() {
  ...
  display.drawString(0, 0, "Test microphone");
  display.drawString(0, 12, "ICS-43434 I2S");
  display.display();
  delay(1000);
}
...
void loop() {
  static int32_t buffer[WINDOW_SIZE * 2];
  static size_t read_size = 0;
  
  i2s_read(I2S_PORT, buffer, sizeof(buffer), &read_size, portMAX_DELAY);
  ...
  display.setFont(ArialMT_Plain_10);
  display.setTextAlignment(TEXT_ALIGN_LEFT);
  display.drawString(0, 0, "G: " + String(dbL, 0) + " dB");
  drawLevelBar(0, 12, dbL);
